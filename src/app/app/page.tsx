  ...
  return (
    <>
      ...
      <div className={`relative min-h-screen overflow-x-hidden transition-colors ${highContrast ? "bg-black text-white" : "bg-transparent text-slate-900"}`}>
        ...
        <main className={`relative mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-8 md:px-8 ${baseFontClasses}`}>
          ...
          <header className={`rounded-3xl border p-6 backdrop-blur-md md:p-8 ${highContrast ? "bg-black border-white/20" : "bg-white/75 border-emerald-900/15"}`}>
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <p className={`text-xs font-semibold uppercase tracking-[0.25em] ${highContrast ? "text-gray-400" : "text-emerald-900/80"}`}>App Workspace</p>
                <h1 className="mt-2 text-3xl font-bold tracking-tight md:text-4xl">Quran Journeys</h1>
              </div>
              <div className="flex items-center gap-2 rounded-xl bg-slate-900/5 p-1">
                ...
                {(["base", "large", "xlarge"] as const).map((s) => (
                  <button key={s} onClick={() => setFontSize(s)} className={`p-2 rounded-lg font-bold text-sm transition ${fontSize === s ? "bg-emerald-900 text-white" : highContrast ? "hover:bg-white/20 text-white" : "hover:bg-slate-200 text-slate-700"}`}>
            ...
            {/* Auth bar */}
            <div className="mt-5 grid gap-3 rounded-2xl border border-emerald-900/10 bg-white/80 p-4">
              {authUser ? (
                <div className="grid w-full gap-3">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <p className="text-sm text-slate-700">Signed in as <span className="font-semibold">{authUser.name}</span> ({authUser.email})</p>
                    <button onClick={() => void handleLogout()} className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">Log Out</button>
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="text-xs text-slate-600">Quran account: {qfConnected ? "Connected" : "Not connected"}{qfConnected && qfExpiry ? ` (expires ${new Date(qfExpiry).toLocaleString()})` : ""}</p>
                    {qfConnected ? (
                      <button type="button" onClick={async () => { await fetch("/api/qf-auth/logout", { method: "POST" }); await refreshQf(); }} className="rounded-xl border border-slate-300 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-50">Disconnect</button>
                    ) : (
                      <button type="button" onClick={() => window.location.assign("/api/qf-auth/start?next=/app")} className="rounded-xl bg-emerald-900 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-emerald-800">Connect Quran Account</button>
                    )}
                  </div>
                </div>
              ) : (
                <div className="flex flex-wrap items-center gap-2">
                  <Link href="/auth?mode=login&next=/app" className="rounded-xl bg-emerald-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-emerald-800">Log In</Link>
                  <Link href="/auth?mode=signup&next=/app" className="rounded-xl bg-amber-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-amber-500">Sign Up</Link>
                  <p className="text-xs text-slate-600">Or continue as guest.</p>
                </div>
              )}
              <p className="text-xs text-slate-500">Sync: {syncMsg}</p>
            </div>
          </header>
            ...
            <>
              ...
              {appView === "session" && !hasCompletedToday && currentDay && (
                <section className="grid gap-6 rounded-3xl border border-emerald-900/15 bg-white/85 p-6 shadow-sm md:p-8">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <h2 className="text-2xl font-semibold">Day {currentDay.dayIndex}: {currentDay.fromAyahKey} – {currentDay.toAyahKey}</h2>
                  ...
                  <div className="flex gap-2 text-xs font-semibold uppercase tracking-[0.15em]">
                    {(["read", "understand", "reflect"] as SessionStep[]).map((s) => (
                      <button key={s} onClick={() => setStep(s)} className={`rounded-full px-4 py-1.5 transition ${step === s ? "bg-emerald-900 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>{s}</button>
                    ))}
                  </div>
